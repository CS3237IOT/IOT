...
# tmp, hmd, x_accel, y_accel, z_accel, gyr_x, gyr_y, gyr_z, x_mag, y_mag, z_mag
para_list = [0] * 13
...
class AccelerometerSensorMovementSensorMPU9250(MovementSensorMPU9250SubService):
    ...
    def cb_sensor(self, data):
        ...
        rawVals = data[3:6]
        global para_list
        para_list[3:6] = [v*self.scale for v in rawVals]
...
class GyroscopeSensorMovementSensorMPU9250(MovementSensorMPU9250SubService):
    ...
    def cb_sensor(self, data):
        ...
        rawVals = data[0:3]
        global para_list
        para_list[6:9] = [v*self.scale for v in rawVals]
...
class HumiditySensor(Sensor):
    ...
    def callback(self, sender: int, data: bytearray):
        ...
        global tmp, hmd, para_list
        temp = -40.0 + 165.0 * (rawT / 65536.0)
        RH = 100.0 * (rawH/65536.0)
        para_list[0] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        para_list[1] = temp
        para_list[2] = RH
...
if __name__ == "__main__":
    ...
    """
    import os
    with open("sensorTag_data.csv","a",newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Time","Temperature","Humidity", "x_accelerate", "y_accelerate", "z_accelerate","x_Gyroscope","y_Gyroscope","z_Gyroscope","x_Magnet","y_Magnet","z_Magnet","Pressure_Millibars"])
