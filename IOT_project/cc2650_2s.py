...
mqtt_pub = Publisher("sensor2/data", "localhost", 1883)
...
class MovementSensorMPU9250(Sensor):
    ...
    def callback(self, sender: int, data: bytearray):
        ...
        """
        Right now count is set as 5 because MC's model use 5s interval for training and our laptop ble transfer is in 1s interval.
        Remeber to adjust accordingly based on how you build the model and the ble laptop transfer sampling rate
        """
...
class HumiditySensor(Sensor):
    ...
    def callback(self, sender: int, data: bytearray):
        ...
        humidity_mean = humidity_mean + RH
        if count == 1:
            arr.append(temp_mean/count)
            arr.append(humidity_mean/count)
            json_msg = {
                'sensor': 2,
                'payload': arr
            }
...
if __name__ == "__main__":
    ...
    address = (
            "9063F7F9-674F-4DCA-9F08-E5031B1166AD"
            if platform.system() != "Darwin"
            else "9063F7F9-674F-4DCA-9F08-E5031B1166AD"
    )
